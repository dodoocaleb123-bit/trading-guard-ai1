    ...
    const rows = summarizeV4Monitoring([
      {
        ...
        direction: "BUY",
        status: "WIN",
        marketSnapshot: JSON.stringify({ fundamentalContext: { eventRisk: "HIGH" }, replacementIntelligence: { decisionTrace: { levelDerivation: { takeProfit: "Next opposing zone is too close for 2R; retained minimum 2R paper target." } } } }),
      },
      {
        asset: "EUR/USD",
        timeframe: "1H",
        direction: "BUY",
        status: "LOSS",
        marketSnapshot: JSON.stringify({ fundamentalContext: { eventRisk: "NORMAL" }, replacementIntelligence: { decisionTrace: { levelDerivation: { takeProfit: "Next opposing zone supports the structural target." } } } }),
      },
    ]);
