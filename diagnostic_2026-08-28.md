...
Implemented the requested v5 zone-persistence correction without changing the hierarchy rules. The scanner now reuses the existing detectWorkflowZones algorithm for every successful asset/timeframe state and stores bounded zone evidence—including zone kind, lower/upper positions, reactions, displacement, freshness, timeframe, support/resistance bands, opposing levels, structure, breakout state, ATR, sample size, candle time, and fetch time. 1H/4H remain WAITING context-only records; 15M/5M retain their Entry Locator state plus the same timeframe’s zone evidence. White AI now reads the persisted zone-evidence envelope and reports concrete zones when present. Focused validation passed 4 files/27 tests, then code-focused validation passed 60 files/249 tests, TypeScript, and production build. The complete suite had one unrelated network-dependent Telegram credential timeout; no application assertion failed.
