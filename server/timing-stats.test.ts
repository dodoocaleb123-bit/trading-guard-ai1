...
describe("best timing paper analytics", () => {
  it("groups signals by UTC hour and separates versions", () => {
    const rows = [
      ...
      { ...base, version: "forex-trading-combined-document-v5", status: "WIN", openedAt: new Date("2026-08-20T09:45:00.000Z") },
    ];
    ...
    const v5 = result.groups.find((group) => group.version === "forex-trading-combined-document-v5" && group.asset === "EUR/USD" && group.timeframe === "1H")!;
    expect(v5.buckets.find((bucket) => bucket.key === "9")).toMatchObject({ generated: 1, resolved: 1, takeProfitHits: 1, winRate: 100 });
  });
